...
const (
	...
	ES_URL      = "http://10.182.0.2:9200" // redeclared?
	BUCKET_NAME = "around-bucket-yumin"
	API_PREFIX  = "/api/v1"
)
...
func handlerSearch(w http.ResponseWriter, r *http.Request) {
	fmt.Println("Received one search request") //debug
...
func saveToGCS(r io.Reader, objectName string) (string, error) {
	...
	//		fmt.Printf("cannot set storage to all users!")
	//		return "", err
	//	}
...
func handlerCluster(w http.ResponseWriter, r *http.Request) {
	...
	query := elastic.NewRangeQuery(term).Gte(0.9) //用face做query在elaticsearch裡面去搜索
